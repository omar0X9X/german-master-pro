...
try{
  const [c,q,v,daily,videos,grammar,zero,studyMethod,errorEngine]=await Promise.all(["./data/curriculum.json","./data/quizzes.json","./data/vocabulary.json","./data/daily-plan.json","./data/videos.json","./data/grammar.json","./data/zero-path.json","./data/study-method.json","./data/error-engine.json"].map(async url=>{const r=await fetch(url,{cache:"no-store"});if(!r.ok)throw new Error(url+" "+r.status);return r.json()}));
  G.data.curriculum=c;G.data.quizzes=q;G.data.vocabulary=v;G.data.daily=daily;G.data.videos=videos;G.data.grammar=grammar;G.data.zero=zero;G.data.studyMethod=studyMethod;G.data.errorEngine=errorEngine;
}catch(err){
